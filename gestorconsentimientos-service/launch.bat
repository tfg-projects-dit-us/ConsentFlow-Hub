...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "gestorconsentimientos-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name gestorconsentimientos-service apps/gestorconsentimientos-service:1.0
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app gestorconsentimientos-service:1.0
		call oc expose svc/gestorconsentimientos-service
	) else (
